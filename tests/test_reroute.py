...
def create_manager(
    ...
    return ultra_ep.Manager(
        ...
        use_quota_solver=use_quota_solver,
    )
...
def test_backward_correctness(mgr, layer_id, T, topk):
    ...
    if mgr.use_quota_solver:
        ...
        probs = torch.randn(T, L, dtype=torch.float32, device="cuda", requires_grad=True)
        ...
        for t in range(min(T, 100)):  # spot-check first 100 tokens
            for l in range(L):
                if routing_map[t, l]:
                    assert abs(grad[t, l].item() - 1.0) < 1e-5, (
                        f"grad[{t},{l}]={grad[t,l].item()} expected 1.0"
                    )
                else:
                    assert grad[t, l].item() == 0.0, (
...
def test_quota_forward_counts(mgr, layer_id, T, topk):
    ...
    if rank_loads.sum().item() > 0:
        ...
        print_rank0(f"  Per-rank load balance: max={max_load}, mean={mean_load:.1f}, imbalance={imbalance:.3f}")
