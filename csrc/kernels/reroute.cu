...
void run_reroute_forward(const bool* routing_map,
    ...
    reroute_forward_scatter_kernel<float, TILE_T, WPB, false><<<grid, block, 0, stream>>>(
        routing_map,
...
void run_reroute_forward_quota(const bool* routing_map,
    ...
    reroute_forward_scatter_kernel<float, TILE_T, WPB, true><<<grid, block, 0, stream>>>(
        routing_map,
        static_cast<const float*>(probs),
        l2p_map,
        lcnts,
...
void reduce_per_rank_loads(const int32_t* loads_per_rank,
                           int32_t* global_loads,
                           int G,
                           int L,
                           cudaStream_t stream) {
    if (G <= 0 || L <= 0)
