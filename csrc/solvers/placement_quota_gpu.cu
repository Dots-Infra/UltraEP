        ...
        if (replica_count > 0) {
            ...
            if constexpr (COMPACT_EOR) {
                ...
                for (int r = 0; r < replica_count; ++r) {
                    ...
                    bool valid = (my_gpu_idx < G) && (my_gpu_slots < num_local_redundant) &&
                    ...
                    int best_slot;
                    bool use_lo =
                        (lo_gpu >= 0) && (hi_gpu < 0 || lo_val < hi_val || (lo_val == hi_val && lo_gpu < hi_gpu));
                    if (use_lo) {
                        best_gpu = lo_gpu;
                        best_slot = lo_slot;
                    } else if (hi_gpu >= 0) {
    ...
    if constexpr (!PARALLEL_D1) {
        ...
        if (tid == 0) {
            ...
            if (need_d1) {
                for (int round = 0; round < QUOTA_D1_MAX_ROUNDS; ++round) {
                    ...
                    bool good_enough = (static_cast<int64_t>(round_max_rank_load) * G * QUOTA_D1_IMBALANCE_DEN <=
                                        total_load * QUOTA_D1_IMBALANCE_NUM);
                    if (good_enough) {
                        break;
                    }
        ...
        for (int round = 0; round < QUOTA_V3_D1_MAX_ROUNDS; ++round) {
            ...
            if (tid == 0) {
                ...
                smem_d1_imbalance_ok = (total_load <= 0) ||
            ...
            for (int expert_local = tid; expert_local < E; expert_local += N_THREADS) {
                ...
                for (int j = 0; j < C; ++j) {
                    ...
                    if (qj > 0) {
                        bool better_donor = (donor_j < 0) || (rank_load > smem_rank_load_d1[donor_rank]) ||
                            (rank_load == smem_rank_load_d1[donor_rank] &&
                             (rank_local < donor_rank || (rank_local == donor_rank && j < donor_j)));
                        if (better_donor) {
                            donor_j = j;
                    ...
                    bool better_receiver = (receiver_j < 0) || (rank_load < smem_rank_load_d1[receiver_rank]) ||
                        (rank_load == smem_rank_load_d1[receiver_rank] &&
                         (rank_local < receiver_rank || (rank_local == receiver_rank && j < receiver_j)));
                    if (better_receiver) {
                        receiver_j = j;
...
void PlacementSolverQuota::solve(const int32_t* expert_loads_gpu,
    ...
    CUDA_RUNTIME_CHECK(cudaMemsetAsync(
        quota_gpu, 0, static_cast<size_t>(num_global_logical_experts_) * max_replicas_dim_ * sizeof(int32_t), stream));
    CUDA_RUNTIME_CHECK(
        cudaMemsetAsync(quota_prefix_gpu,
                        0,
                        static_cast<size_t>(num_global_logical_experts_) * max_replicas_dim_ * sizeof(int32_t),
                        stream));
    CUDA_RUNTIME_CHECK(
        cudaMemsetAsync(rank_quota_prefix_gpu,
                        0,
                        static_cast<size_t>(num_global_logical_experts_) * max_replicas_dim_ * sizeof(int32_t),
                        stream));
